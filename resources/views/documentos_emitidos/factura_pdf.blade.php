...
<body>
  <div class="ticket">
    ...
    <p class="">
       ...
       Fecha Factura: {{date('d-m-Y', strtotime($venta->fecha_factura))}}</p>
    <p>Cliente: {{strtoupper($venta->razon_social)}}<br>
       RUC/CI: {{$venta->documento}}</p>
    <div class="line"></div>
    <div class="left-aligned">
        <div class="item">
          <p class="description">COBRO DE ESTACIONAMIENTO</p>
          <div class="details">
            <span><strong>Cant:</strong> {{ number_format($venta->monto_factura / 10000, 2, ',', '.') }}</span>
            <span><strong>Precio:</strong> {{ number_format(10000, 0, '', '.') }}</span>
            <span><strong>Total:</strong> {{ number_format($venta->monto_factura, 0, '', '.') }}</span>
          </div>
        </div>
    </div>
    <div class="line"></div>
    <table>
      <tbody>
        ...
        <tr>
          <td>Gravadas 10%</td>
          <td style="text-align: right;">{{number_format($venta->monto_factura - round($venta->monto_factura / 11), 0, '', '.')}}</td>
        </tr>
        <tr>
          <td>IVA 10%</td>
          <td style="text-align: right;">{{number_format(round($venta->monto_factura / 11), 0, '', '.')}}</td>
        </tr>
        <tr>
          <td>Gravadas 5%</td>
          <td style="text-align: right;">0</td>
    ...
    <div class="line"></div>
    <p class="right-aligned bold">Total: {{number_format($venta->monto_factura, 0, '', '.')}}</p>
    <div class="centered line"></div>
    <div class="centered">
